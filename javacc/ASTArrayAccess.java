  ...
  public void checkArrayOperand(SimpleNode operand, Type type) throws SemanticsException {
    if(operand instanceof ASTIdentifier) {  //Check if the node is a variable
      ...
      if(table.checkSymbol(name)) { //And check if the identifier already has a symbol declared
        ...
        if(type != leftSymbol.getType()) throw new SemanticsException("Identifier '" + name + "' is not of type: int array");
      }
    ...
    } else if (! (operand instanceof ASTMethodCall)) new SemanticsException("Invalid expression"); //TODO: REMOVE METHOD CALL CONDITION
  }
}
